...
# 生成JSON条目
entries = []
for i, (root, phonetic, meaning, words_list) in enumerate(roots_data, start=201):
    entry = {
        "id": i,
        "root": root,
        "phonetic": phonetic,
    ...
    for word in words_list:
        word_entry = {
            ...
            "rootMeaning": meaning,
            "rootPhrases": words_list[:4] if len(words_list) >= 4 else words_list,
            "wordPhrases": words_list[:4] if len(words_list) >= 4 else words_list
        }
        entry["words"].append(word_entry)

    # 添加短语
    entry["phrases"] = [f"{w} (待翻译)" for w in (words_list[:4] if len(words_list) >= 4 else words_list)]
...
# 移除新条目的 [ 和 ]
new_entries_json = new_entries_json.strip()
if new_entries_json.startswith('['):
    new_entries_json = new_entries_json[1:]
if new_entries_json.endswith(']'):
    new_entries_json = new_entries_json[:-1]

# 合并
if content.strip().endswith('}'):
    content = content + ',\n'
content = content + new_entries_json + '\n]'

# 写入文件
with open('src/data/mechanical.json', 'w', encoding='utf-8') as f:
    f.write(content)

print(f"成功添加 {len(entries)} 个新词根到 mechanical.json")
