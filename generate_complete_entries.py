...
def main():
    ...
    max_existing_id = max(e['id'] for e in existing_data)
    print(f"Existing entries: {len(existing_data)}, Max ID: {max_existing_id}")

    # 所有200个新条目的数据 (801-1000)
    # 格式: (id, root, phonetic, pos, freq, meaning, [(word, phonetic, pos, meaning, memory_tip, phrases), ...], phrases, mnemonic)
    new_entries_data = [
        # ========== 801-810: 基础材料科学 ==========
    ...
    print(f"Prepared {len(new_entries_data)} new entries")
    ...
    for data in new_entries_data:
        ...
        words = []
        for w in words_list:
            words.append(create_word_entry(w[0], w[1], w[2], w[3], w[4], w[5] if len(w) > 5 else phrases))
        new_entries.append(create_entry(id_num, root, phonetic, pos, freq, meaning, words, phrases, mnemonic))

    # 合并数据
    combined_data = existing_data + new_entries

    print(f"Total entries after merge: {len(combined_data)}")
    print(f"New entries ID range: {new_entries[0]['id']} - {new_entries[-1]['id']}")
...
if __name__ == "__main__":
    count = main()
    print(f"\nAdded {count} new entries to semiconductor.json")
